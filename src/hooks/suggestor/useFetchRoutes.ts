...
const useFetchRoutes = () => {
  ...
  const fetchRoutes = (collateral: string, loan: string, chainId: number) => {
    setLoading(true);
    setError(null);
    try {
      console.log("Starting Route search");
      const result = findRoutes(collateral, loan, chainId);
      console.log("Fetched Route Data in hook:", result);
      setRouteData(result);
    } catch (err) {
  ...
  return { routeData, loading, error, fetchRoutes };
};
